...
#include "Specifications/GUI/BBJobWidgetSpecification.h"
#include "Tests/BBTestUtil.h"

BEGIN_DEFINE_SPEC(UBBJobWidgetSpecificationSpec, "SaltLakeCity.Specifications.GUI.JobWidgetSpecification", EAutomationTestFlags::ProductFilter | EAutomationTestFlags::ApplicationContextMask)
	...
	// SUT
	UPROPERTY()
	UBBJobWidgetSpecification * WidgetSpecification = nullptr;

END_DEFINE_SPEC(UBBJobWidgetSpecificationSpec)

void UBBJobWidgetSpecificationSpec::Define()
{
	UBBTestUtil::GetTestClasses<UBBJobWidgetSpecification>(FString(TEXT("/Game/Specifications/GUI")), WidgetSpecificationClasses);
	...
	for (UClass * & WidgetSpecificationClass : WidgetSpecificationClasses)
	{
		...
		{
			It("Given a new GUI model that is not a job model, expect the specification to evaluate it negatively.", [this, WidgetSpecificationClass]()
			{
				WidgetSpecification = NewObject<UBBJobWidgetSpecification>(TestWorld, WidgetSpecificationClass);
			...
			It("Given a new GUI model that is a job model, expect the specification to evaluate it correctly.", [this, WidgetSpecificationClass]()
			{
				WidgetSpecification = NewObject<UBBJobWidgetSpecification>(TestWorld, WidgetSpecificationClass);

				UTEST_TRUE("Widget Specification is valid", IsValid(WidgetSpecification))

				UBBJobModelStub * TestModel = NewObject<UBBJobModelStub>(WidgetSpecification, UBBJobModelStub::StaticClass());
					
				UTEST_TRUE("Test Model is valid", IsValid(TestModel))

				UBBJobWidgetStub * TestView = CreateWidget<UBBJobWidgetStub>(TestWorld, UBBJobWidgetStub::StaticClass());
